      ...
      <div class="flex flex-col sm:flex-row items-center gap-3">
        <form method="GET" action="{{ route('admin.progress.index') }}" class="flex gap-3 w-full sm:w-auto">
          <input type="text" name="search" value="{{ request('search') }}" class="px-4 py-2 w-full sm:w-64 border border-gray-300 rounded-lg 
              ...
              bg-white/50 backdrop-blur-sm placeholder-gray-500" placeholder="Cari nama pegawai atau tugas...">
          ...
          <select name="status" onchange="this.form.submit()"
            ...
            <option value="on_progress" {{ request('status') == 'on_progress' ? 'selected' : '' }}>Sedang Dikerjakan
            </option>
            <option value="waiting_approval" {{ request('status') == 'waiting_approval' ? 'selected' : '' }}>Menunggu
              Persetujuan</option>
        ...
        {{-- Tombol Export --}}
        <a href="{{ route('admin.progress.export') }}" class="inline-flex items-center px-4 py-2 rounded-lg border border-green-400 text-green-600 font-medium
          ...
          @forelse($tugas as $index => $t)
            <tr id="tugas-{{ $t['id'] }}" class="text-center hover:bg-gray-50">
              ...
              <td class="px-3 py-2 border">
                ...
                @if($status === 'pending')
                  <span class="inline-block px-2 py-1 text-xs font-semibold text-white bg-red-500 rounded">Belum
                    Dikerjakan</span>
                @elseif($status === 'waiting_approval')
                  <span class="inline-block px-2 py-1 text-xs font-semibold text-black bg-yellow-300 rounded">Menunggu
                    Persetujuan</span>
                @elseif($status === 'on_progress')
                  <span class="inline-block px-2 py-1 text-xs font-semibold text-black bg-yellow-300 rounded">Sedang
                    Dikerjakan</span>
                @else
              ...
              <td class="px-3 py-2 border">
                @if(!$t['isApproved'] && auth()->user()->pegawai && $t['asal'] === auth()->user()->pegawai->nama)
                  <form method="POST" action="{{ route('admin.progress.approve', $t['id']) }}">
                    @csrf
                    @method('PATCH')
                    <button type="submit" class="flex items-center px-3 py-1.5 text-xs font-semibold rounded border border-blue-400 text-blue-600
