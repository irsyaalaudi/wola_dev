...
@section('content')
    ...
    <div x-data="{ tambahModal: false, editModal: null, openImport: false }" class="bg-white rounded-2xl p-6 mb-12 border border-gray-200">
        ...
        <div class="overflow-x-auto rounded-xl border border-gray-200 shadow-sm bg-white">
            <table class="w-full table-auto text-sm text-gray-700">
                ...
                <tbody>
                            ...
                            <td class="p-3 border">
                                <div class="flex items-center justify-center gap-2">
                                    @if ($t->status === 'pending')
                                        ...
                                        {{-- Tombol Hapus --}}
                                        <form action="{{ route('admin.pekerjaan.destroy', $t->id) }}" method="POST"
                                            class="inline">
                                            @csrf @method('DELETE')
                                    ...
                                    @else
                                        <div
                                            class="flex items-center gap-1 text-gray-400 bg-gray-50 px-2 py-1 rounded border border-gray-100">
                                    ...
                                    <div class="space-y-4">
                                        ...
                                        <div>
                                            ...
                                            <select name="jenis_pekerjaan_id"
                                                class="w-full border rounded px-3 py-2 jenis-edit"
                                                data-id="{{ $t->id }}" required>
                                                <option value="">-- Pilih Jenis Pekerjaan --</option>
                                                @foreach ($jenisPekerjaanModal as $jp)
                                                    <option value="{{ $jp->id }}" data-satuan="{{ $jp->satuan }}"
                                                        {{ $t->jenis_pekerjaan_id == $jp->id ? 'selected' : '' }}>
                                                        {{ $jp->nama_pekerjaan }}
                                                        ({{ $jp->teams->pluck('nama_tim')->implode(', ') ?: '-' }})
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="text-center border px-4 py-6 text-gray-500">Belum ada tugas yang
                                ditambahkan.</td>
                        </tr>
                    @endforelse
                </tbody>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <select name="pegawai_id" class="w-full border rounded px-3 py-2" required>
                                <option value="">-- Pilih Pegawai --</option>
                                @foreach ($pegawai as $p)
                                    <option value="{{ $p->id }}">{{ $p->user->name }} - {{ $p->jabatan }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            <label class="block mb-1">Jenis Pekerjaan</label>
                            <select name="jenis_pekerjaan_id" id="jenisTambah" class="w-full border rounded px-3 py-2"
                                required>
                                <option value="">-- Pilih Jenis Pekerjaan --</option>
                                @foreach ($jenisPekerjaanModal as $jp)
                                    <option value="{{ $jp->id }}" data-satuan="{{ $jp->satuan }}">
                        ...
                        <div>
                            <label class="block mb-1">Target</label>
                            <input type="number" name="target" id="targetTambah"
                                class="w-full border rounded px-3 py-2" required>
                        </div>
        ...
        <div x-show="openImport" x-cloak class="fixed inset-0 flex items-center justify-center bg-black/50 z-50">

            <div class="bg-white rounded-xl p-6 w-96" @click.away="openImport = false">
                ...
                <form action="{{ route('admin.pekerjaan.import') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <a href="{{ route('admin.pekerjaan.template') }}"
                        class="inline-flex items-center px-3 py-2 rounded-lg border border-purple-400 text-purple-600 text-sm
   hover:bg-purple-600 hover:text-white transition mb-4">
                        <i class="fas fa-download mr-2"></i> Download Template
                    </a>

                    <div class="flex justify-end gap-2">

                        <button type="button" @click="openImport=false" class="px-4 py-2 border rounded">
                            Batal
                        </button>
