...
class DashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($bulan) {
            ...
            $endDate   = \Carbon\Carbon::create($year, $bulan, 1)->endOfMonth();
        } elseif ($tahun) {
        ...
        $pegawais = Pegawai::with([
            ...
            // },
            'tugas' => function ($q) use ($startDate, $endDate) {
        ...
        $data = $pegawais->map(function ($pegawai) use ($teams, $startDate, $endDate) {

            $teamsData = $teams->map(function ($team) use ($pegawai, $startDate, $endDate) {
        ...
        if ($startDate && $endDate) {
            ...
            $tasks->whereDate('start_date', '<=', $endDate)
                ->whereDate('deadline', '>=', $startDate);
        }
            ...
            new class($pegawais, $teams, $bulan, $tahun) implements
                ...
                public function map($pegawai): array
                {
                    $teamsData = $this->teams->map(function ($team) use ($pegawai) {
                        ...
                        $year = $this->tahun ?? now()->year;

                        if ($this->bulan) {

                            $startDate = \Carbon\Carbon::create($year, $this->bulan, 1)->startOfMonth();
                            $endDate   = \Carbon\Carbon::create($year, $this->bulan, 1)->endOfMonth();
                        } elseif ($this->tahun) {
                    ...
                    if ($grandTarget == 0) {
                        ...
                        $score = round(($grandRealisasi / $grandTarget) * 100, 2);
